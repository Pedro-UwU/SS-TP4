...
public class Beeman extends Integration{
    ...
    public Particle update() {
        ...
        Vector2D acc = force.mul(1.0 / particle.mass);
        ...
        */
        ...
        Vector2D nextPos = particle.pos
                ...
                //2/3*a(t)*dt^2
                .add(acc.mul(2.0/3 * delta_t_square))
                //1/6*a(t-dt)*dt^2
                .sub(prevAcc.mul(1.0/6 * delta_t_square));
        // v(t) + 3/2 a(t) * dt - 1/2 a(t-dt)*dt
        Vector2D predictedVel = particle.vel
                .add(acc.mul(3.0/2 * delta_t ))
                .sub(prevAcc.mul(  1.0/2 * delta_t ));
        ...
        particle.vel = particle.vel
                .add( nextAcc.mul(1.0/3 * delta_t) )
                .add( acc.mul(5.0/6 * delta_t ) )
                .sub( prevAcc.mul(1.0/6 * delta_t ));
